...
module Shumway.AVMX.AS.flash.text.engine {
	...
	export class TextElement extends flash.text.engine.ContentElement {
		...
		set text(value: string) {
			...
			release || notImplemented("public flash.text.engine.TextElement::set text");
			return;
			// this._text = value;
		}
		...
		replaceText(beginIndex: number /*int*/, endIndex: number /*int*/, newText: string): void {
			...
			endIndex = endIndex | 0;
			newText = axCoerceString(newText);
			release || notImplemented("public flash.text.engine.TextElement::replaceText");
			return;
		}
	}
}
