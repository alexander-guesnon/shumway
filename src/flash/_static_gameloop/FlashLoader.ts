module Shumway {
	...
	export class FlashLoader {
		...
		getRootLoader(): Loader {
			...
			let loader = new this.context.sec.flash.display.Loader();
		...
		processEarlyEvents() {
			...
			for (let i = 0; i < queue.length; i++) {
				...
				if (instance._loadStatus === LoadStatus.Opened && instance._content) {
					...
					try {
						loaderInfo.dispatchEvent(this.context.events.getInstance(Event.INIT));
					} catch (e) {
					...
					if (instance === this._rootLoader) {
						...
						try {
							loaderInfo.dispatchEvent(new this.context.sec.flash.events.ProgressEvent(
				...
				if (instance._loadStatus === LoadStatus.Initialized &&
					...
					try {
						loaderInfo.dispatchEvent(this.context.events.getInstance(Event.COMPLETE));
					} catch (e) {
		...
		processLateEvents() {
			...
			for (let i = 0; i < queue.length; i++) {
				...
				let progressEventCtor = this.context.sec.flash.events.ProgressEvent;
				if (instance._loadStatus === LoadStatus.Unloaded) {
					...
					if (instance._loadingType === LoadingType.External) {
						...
						try {
							loaderInfo.dispatchEvent(this.context.events.getInstance(Event.OPEN));
						} catch (e) {
					...
					try {
						loaderInfo.dispatchEvent(new progressEventCtor(ProgressEvent.PROGRESS,
