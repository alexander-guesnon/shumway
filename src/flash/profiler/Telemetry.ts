...
module Shumway.AVMX.AS.flash.profiler {
	...
	export class Telemetry extends ASObject {
		...
		get spanMarker(): number {
			release || notImplemented("public flash.profiler.Telemetry::get spanMarker");
			return;
			// return this._spanMarker;
		}

		get connected(): boolean {
			release || notImplemented("public flash.profiler.Telemetry::get connected");
			return;
		...
		static sendMetric(metric: string, value: any): void {
			...
			release || notImplemented("public flash.profiler.Telemetry::static sendMetric");
			return;
		}

		static sendSpanMetric(metric: string, startSpanMarker: number, value: any = null): void {
			metric = axCoerceString(metric);
			startSpanMarker = +startSpanMarker;
			release || notImplemented("public flash.profiler.Telemetry::static sendSpanMetric");
			return;
		}
		...
		static registerCommandHandler(commandName: string, handler: ASFunction): boolean {
			...
			handler = handler;
			release || notImplemented("public flash.profiler.Telemetry::static registerCommandHandler");
			return;
		}

		static unregisterCommandHandler(commandName: string): boolean {
			commandName = axCoerceString(commandName);
			release || notImplemented("public flash.profiler.Telemetry::static unregisterCommandHandler");
			return;
		}
