...
module Shumway.AVMX.AS.flash.display3D {
	...
	export class Context3D extends flash.events.EventDispatcher {
		...
		set enableErrorChecking(toggle: boolean) {
			toggle = !!toggle;
			release || notImplemented("public flash.display3D.Context3D::set enableErrorChecking");
			return;
			// this._enableErrorChecking = toggle;
		}

		dispose(): void {
			release || notImplemented("public flash.display3D.Context3D::dispose");
			return;
		}
		...
		configureBackBuffer(width: number /*int*/, height: number /*int*/, antiAlias: number /*int*/, enableDepthAndStencil: boolean = true, wantsBestResolution: boolean = false): void {
			...
			wantsBestResolution = !!wantsBestResolution;
			release || notImplemented("public flash.display3D.Context3D::configureBackBuffer");
			return;
		}
		...
		clear(red: number = 0, green: number = 0, blue: number = 0, alpha: number = 1, depth: number = 1, stencil: number /*uint*/ = 0, mask: number /*uint*/ = 4294967295): void {
			...
			stencil = stencil >>> 0;
			mask = mask >>> 0;
			release || notImplemented("public flash.display3D.Context3D::clear");
			return;
		}
		...
		drawTriangles(indexBuffer: flash.display3D.IndexBuffer3D, firstIndex: number /*int*/ = 0, numTriangles: number /*int*/ = -1): void {
			...
			release || notImplemented("public flash.display3D.Context3D::drawTriangles");
			return;
		}

		present(): void {
			release || notImplemented("public flash.display3D.Context3D::present");
			return;
		}

		setProgram(program: flash.display3D.Program3D): void {
			program = program;
			release || notImplemented("public flash.display3D.Context3D::setProgram");
			return;
		}
		...
		setProgramConstantsFromVector(programType: string, firstRegister: number /*int*/, data: any /*ASVector*/, numRegisters: number /*int*/ = -1): void {
			...
			release || notImplemented("public flash.display3D.Context3D::setProgramConstantsFromVector");
			return;
		}
		...
		setProgramConstantsFromMatrix(programType: string, firstRegister: number /*int*/, matrix: flash.geom.Matrix3D, transposedMatrix: boolean = false): void {
			...
			release || notImplemented("public flash.display3D.Context3D::setProgramConstantsFromMatrix");
			return;
		}
		...
		setProgramConstantsFromByteArray(programType: string, firstRegister: number /*int*/, numRegisters: number /*int*/, data: flash.utils.ByteArray, byteArrayOffset: number /*uint*/): void {
			...
			release || notImplemented("public flash.display3D.Context3D::setProgramConstantsFromByteArray");
			return;
		}
		...
		setVertexBufferAt(index: number /*int*/, buffer: flash.display3D.VertexBuffer3D, bufferOffset: number /*int*/ = 0, format: string = "float4"): void {
			...
			release || notImplemented("public flash.display3D.Context3D::setVertexBufferAt");
			return;
		}

		setBlendFactors(sourceFactor: string, destinationFactor: string): void {
			sourceFactor = axCoerceString(sourceFactor);
			destinationFactor = axCoerceString(destinationFactor);
			release || notImplemented("public flash.display3D.Context3D::setBlendFactors");
			return;
		}
		...
		setColorMask(red: boolean, green: boolean, blue: boolean, alpha: boolean): void {
			...
			blue = !!blue;
			alpha = !!alpha;
			release || notImplemented("public flash.display3D.Context3D::setColorMask");
			return;
		}

		setDepthTest(depthMask: boolean, passCompareMode: string): void {
			depthMask = !!depthMask;
			passCompareMode = axCoerceString(passCompareMode);
			release || notImplemented("public flash.display3D.Context3D::setDepthTest");
			return;
		}

		setCulling(triangleFaceToCull: string): void {
			triangleFaceToCull = axCoerceString(triangleFaceToCull);
			release || notImplemented("public flash.display3D.Context3D::setCulling");
			return;
		}
		...
		setStencilActions(triangleFace: string = "frontAndBack", compareMode: string = "always", actionOnBothPass: string = "keep", actionOnDepthFail: string = "keep", actionOnDepthPassStencilFail: string = "keep"): void {
			triangleFace = axCoerceString(triangleFace);
			compareMode = axCoerceString(compareMode);
			actionOnBothPass = axCoerceString(actionOnBothPass);
			actionOnDepthFail = axCoerceString(actionOnDepthFail);
			actionOnDepthPassStencilFail = axCoerceString(actionOnDepthPassStencilFail);
			release || notImplemented("public flash.display3D.Context3D::setStencilActions");
			return;
		}
		...
		setStencilReferenceValue(referenceValue: number /*uint*/, readMask: number /*uint*/ = 255, writeMask: number /*uint*/ = 255): void {
			...
			release || notImplemented("public flash.display3D.Context3D::setStencilReferenceValue");
			return;
		}

		setScissorRectangle(rectangle: flash.geom.Rectangle): void {
			rectangle = rectangle;
			release || notImplemented("public flash.display3D.Context3D::setScissorRectangle");
			return;
		}
		...
		drawToBitmapData(destination: flash.display.BitmapData): void {
			...
			release || notImplemented("public flash.display3D.Context3D::drawToBitmapData");
			return;
		}
		...
		setRenderToTextureInternal(texture: flash.display3D.textures.TextureBase, targetType: number /*int*/,
			...
			release || notImplemented("public flash.display3D.Context3D::setRenderToTextureInternal");
			return;
		}
		...
		setTextureInternal(sampler: number /*int*/, texture: flash.display3D.textures.Texture): void {
			sampler = sampler | 0;
			texture = texture;
			release || notImplemented("public flash.display3D.Context3D::setTextureInternal");
			return;
		}
		...
		setCubeTextureInternal(sampler: number /*int*/, texture: flash.display3D.textures.CubeTexture): void {
			sampler = sampler | 0;
			texture = texture;
			release || notImplemented("public flash.display3D.Context3D::setCubeTextureInternal");
			return;
		}
	}
}
