...
module Shumway.AVMX.AS {
	...
	export class Float64Vector extends BaseVector {
		...
		static classInitializer() {
			...
			let asProto: any = Float64Vector.prototype;
		...
		_ensureCapacity(length: number) {
			...
			let buffer = new Float64Array(newCapacity);
		...
		concat() {
			...
			for (let i = 0; i < arguments.length; i++) {
				let vector: Float64Vector = arguments[i];
			...
			let result = new this.sec.Float64Vector(length);
		...
		slice(start = 0, end = 0x7fffffff) {
			...
			let result = new this.sec.Float64Vector(last - first, this.fixed);
		...
		splice(start: number, deleteCount_: number /*, ...items: number[] */) {
			...
			let result = new this.sec.Float64Vector(deleteCount, this.fixed);
			if (deleteCount > 0) {
