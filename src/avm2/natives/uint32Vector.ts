...
module Shumway.AVMX.AS {
	...
	export class Uint32Vector extends BaseVector {
		...
		static classInitializer() {
			...
			let asProto: any = Uint32Vector.prototype;
		...
		_ensureCapacity(length: number) {
			...
			let buffer = new Uint32Array(newCapacity);
		...
		concat() {
			...
			for (let i = 0; i < arguments.length; i++) {
				let vector: Uint32Vector = arguments[i];
			...
			let result = new this.sec.Uint32Vector(length);
		...
		filter(callback: any, thisObject: any) {
			let v = new this.sec.Uint32Vector();
			if (!this.checkVectorMethodArgs(callback, thisObject)) {
				return v;
			}
			for (let i = 0; i < this._length; i++) {
		...
		slice(start = 0, end = 0x7fffffff) {
			...
			let result = new this.sec.Uint32Vector(last - first, this.fixed);
		...
		splice(start: number, deleteCount_: number /*, ...items: number[] */) {
			...
			let result = new this.sec.Uint32Vector(deleteCount, this.fixed);
			if (deleteCount > 0) {
